...
class Book {
  ...
  const setupRealTimeListener = () => {
    const userBooksRef = firebase.database().ref('books').orderByChild('ownerId').equalTo(auth.currentUser.uid);
    userBooksRef.on('value', (snapshot) => {
      const booksData = snapshot.val();
      library.books = booksData ? Object.values(booksData).map(book => JSONToBook(book)) : [];
      updateBooksGrid();
    });
  }
  ...
  const addBookDB = (newBook) => {
    ...
    newBookRef.set(bookToDoc(newBook));
  }
  ...
  const toggleBookIsReadDB = async (book) => {
    const bookId = await getBookIdDB(book.title);
    if (bookId) {
      firebase.database().ref('books/' + bookId).update({ isRead: !book.isRead });
    }
  }
